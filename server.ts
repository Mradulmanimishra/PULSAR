...
async function startServer() {
  ...
  const PLAYER_COLORS = ['#ff0055', '#00ff88', '#ffff00', '#ff00ff', '#00ffff', '#ff8800', '#88ff00', '#0088ff'];

  io.on('connection', (socket) => {
    console.log('User connected:', socket.id);

    // FIX: accept optional name from client payload
    socket.on('joinGame', (payload?: { name?: string }) => {
      if (Object.keys(players).length >= MAX_PLAYERS) {
        socket.emit('gameError', 'Server is full (60/60 players)');
        return;
      }

      const color = PLAYER_COLORS[Object.keys(players).length % PLAYER_COLORS.length];

      // Use client-supplied name if provided and non-empty, otherwise fall back
      const rawName = payload?.name?.trim();
      const playerName = rawName ? rawName.slice(0, 20) : `Player ${playerCounter++}`;
